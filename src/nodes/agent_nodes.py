...
# src/nodes/agent_nodes.py
"""
LLM setup and system prompt used by the agent.

Note: the manual agent_node / tool_node / should_continue functions and the
model.bind_tools(tools) call are no longer needed here — create_agent()
(in src/graph/builder.py) builds and runs that whole loop internally.
"""
